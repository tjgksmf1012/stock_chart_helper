...
수집 시작 전에 run_backtest()를 직접 기다려서 패턴별 실제 백테스트 통계부터
준비한다 — 안 그러면 앱이 평소 쓰는 get_pattern_stats_map()이 캐시가 비어 있을
때 백그라운드로만 계산을 던지고 즉시 패턴 타입과 무관한 범용 기본 승률을
돌려주므로, 이 스크립트처럼 짧게 끝나는 프로세스에서는 그 계산이 끝나기 전에
수집이 다 끝나버려 캘리브레이션 결과 자체가 무의미해진다.
...
async def main() -> None:
    ...
    # get_pattern_stats_map()은 캐시가 비어 있으면 run_backtest()를 백그라운드
    # task로만 던지고 즉시 범용 기본 승률(_DEFAULT_WIN_RATES, 패턴 타입 상관없이
    # 52~62%로 낙관적)을 돌려준다 -- 그 백그라운드 작업이 끝나길 기다리지 않는다.
    # 이 스크립트가 짧게 끝나는 프로세스라 그 작업이 끝나기 전에 전체 수집이
    # 끝나버리면, empirical_up(백테스트 실제 승률) 컴포넌트가 패턴 타입과 무관한
