...
def _detect(df: pd.DataFrame):
    return PatternEngine()._detect_momentum_breakout(df, swings=[], regime_fit=0.5, timeframe="1d")


class TestMomentumBreakoutDetection:
    def test_confirmed_breakout_with_volume_expansion(self):
        base = [100.0] * 45  # flat consolidation, resistance ~= 100 * 1.005 (high)
        base_volumes = [1_000_000.0] * 45
        # breakout bar: close ~3% above resistance (within the +6% detection window)
        # with a clear volume surge
        closes = base + [103.5]
    ...
    def test_no_detection_in_a_downtrend(self):
        ...
        closes = [100.0 - i * 0.5 for i in range(41)]
        volumes = [1_000_000.0] * 41
        df = _build_df(closes, volumes)
