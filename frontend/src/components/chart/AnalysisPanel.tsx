...
import { cn, fmtNumber, fmtPct, fmtPrice, getPatternBias, PATTERN_NAMES, STATE_COLORS, STATE_LABELS } from '@/lib/utils'
...
function statsTimeframeLabel(value: string | null | undefined): string {
  switch (value) {
    case '1mo':
      return '월봉'
    case '1wk':
...
function patternActionText(pattern: PatternInfo, analysis: AnalysisResult): string {
  ...
  if (pattern.state === 'forming') {
    return bias === 'bullish'
      ? '패턴이 아직 완성 전입니다. 목선 돌파와 거래량 반응이 실제로 따라오는지 확인하는 구간입니다.'
      : '패턴이 아직 완성 전입니다. 지지 이탈과 추세 약화가 실제로 이어지는지 확인하는 구간입니다.'
  }

  if (pattern.state === 'armed') {
    return bias === 'bullish'
      ? '확인 직전 단계입니다. 추격보다 돌파 유지 여부와 눌림 지지 반응을 먼저 보는 편이 안전합니다.'
      : '하락 확인 직전 단계입니다. 급한 진입보다 지지 붕괴와 반등 실패가 실제로 나오는지 먼저 확인하는 편이 좋습니다.'
  }

  if (analysis.p_up >= 0.6) {
    return '이미 확인된 패턴으로 해석되고 있습니다. 목표가보다 먼저 무효화 기준을 지키는지가 더 중요합니다.'
  }

  return '패턴은 감지됐지만 확신 구간은 아닙니다. 확률, 유사도, 무효화 기준을 함께 보고 보수적으로 해석하는 편이 좋습니다.'
}
  ...
  if (analysis.no_signal_flag) {
    return (
      <Card className="space-y-3">
        ...
        <p className="text-sm text-foreground">{analysis.no_signal_reason}</p>
        ...
        <div className="space-y-2">
          ...
          <StatRow label="분석 바 수" value={`${analysis.available_bars}개`} />
          <StatRow label="유동성" value={fmtPct(analysis.liquidity_score)} />
          <StatRow label="평균 거래대금" value={`${fmtNumber(analysis.avg_turnover_billion)}억`} />
        </div>
      </Card>
    )
  }
    ...
    <div className="space-y-3">
      <Card>
        ...
        <div className="space-y-3">
          ...
          {analysis.fetch_message && (
            <p className="text-xs leading-relaxed text-muted-foreground">{analysis.fetch_message}</p>
          )}
          <p className="text-xs leading-relaxed text-muted-foreground">{analysis.reason_summary}</p>
        </div>
      </Card>
      ...
      {bestPattern && (
        <Card className="space-y-3">
          ...
          <div className="space-y-2">
            {bestPattern.neckline && <StatRow label="목선" value={fmtPrice(bestPattern.neckline)} />}
      ...
      <Card>
        ...
        <div className="space-y-2">
          ...
          <StatRow label="유사 패턴 표본" value={`${analysis.sample_size}건`} />
          <StatRow label="통계 기준" value={statsTimeframeLabel(analysis.stats_timeframe)} />
          <StatRow label="분석 바 수" value={`${analysis.available_bars}개`} />
          {analysis.bars_since_signal !== null && (
            <StatRow label="신호 이후 경과 바" value={`${analysis.bars_since_signal}개`} />
          )}
        </div>
      </Card>
      ...
      <Card className="space-y-3">
        ...
        <p className="text-xs leading-relaxed text-muted-foreground">
          분봉은 공급 소스 상태에 따라 저장된 데이터나 fallback 데이터가 사용될 수 있습니다.
          특히 KIS 없이 보는 15분 이하 차트는 일봉보다 보수적으로 해석하는 편이 좋습니다.
        </p>
      </Card>
      ...
      <Card className="space-y-3">
        ...
        <p className="text-xs leading-relaxed text-muted-foreground">
          이 화면은 패턴 기반 분석 보조 도구입니다. 확률이 높아도 추세 강도, 거래대금, 종목 특성에 따라 결과가 달라질 수 있으니
          목선과 무효화 기준을 함께 보고 해석하는 편이 안전합니다.
        </p>
      </Card>
    </div>
  )
