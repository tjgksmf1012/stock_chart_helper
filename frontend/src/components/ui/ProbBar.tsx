...
export function ProbBar({ p_up, p_down, size = 'sm' }: ProbBarProps) {
  ...
  return (
    <div className="space-y-1">
      <div className={`flex rounded-full overflow-hidden ${h} bg-muted w-full`}>
      ...
      <div className="flex justify-between text-xs text-muted-foreground">
        <span className="text-green-400">↑ {fmtPct(p_up)}</span>
        <span className="text-red-400">↓ {fmtPct(p_down)}</span>
      </div>
    </div>
  )
}
